...
interface Sockets (m : Type -> Type) where
  ...
  bind : (sock : Label) -> (addr : Maybe SocketAddress) -> (port : Port) ->
         Vars m (Either () ()) 
                [sock ::: Sock Closed]
  ...
  connect : (sock : Label) -> SocketAddress -> Port ->
            Vars m (Either () ())
                   [sock ::: Sock Closed]
                   (either (const [sock ::: Sock Closed]) 
...
rndServer : (ConsoleIO io, foo : Sockets io) =>
            (sock : Label) -> (seed : Integer) ->
            Vars io () [sock ::: Sock @{foo} Listening] 
                       (const [sock ::: Sock @{foo} Closed])
rndServer sock seed = 
